...
public class NetUtils {
    ...
    /* public static void main(String[] args) {
        ...
        destHostPorts.put("192.168.1.151", 2181);
        InetAddress localAddress = NetUtils.getLocalAddress(destHostPorts);
     ...
     * @return
     */
    public static String getLocalIpByZkUrl(String zkUrl) {
        ConnectStringParser connectStringParser = new ConnectStringParser(zkUrl);
        Map<String, Integer> destHostPorts = new HashMap<>();
        for (InetSocketAddress h : connectStringParser.getServerAddresses()) {
            destHostPorts.put(h.getHostString(), h.getPort());
        }
        String localAddress = NetUtils.getLocalAddress(destHostPorts).getHostAddress();
        return localAddress;
    }

    /**
     * <pre>
     * 查找策略： hostname对应的ip -->  首先看是否已经查到ip --> 根据连接目标端口得到的本地ip --> 轮询网卡
     * </pre>
     * 
     * @return loca ip
     */
        ...
        for (Map.Entry<String, Integer> entry : destHostPorts.entrySet()) {
            ...
            try {
                ...
                logger.warn(String.format(
                        "Failed to retriving local address by connecting to dest host:port(%s:%s) false, e=%s", host,
                        port, e));
            }
        }
        return null;
