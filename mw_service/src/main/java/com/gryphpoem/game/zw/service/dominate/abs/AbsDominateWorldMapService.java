...
public abstract class AbsDominateWorldMapService implements IDominateWorldMapService {
    ...
    public GamePb8.AttackDominateCityRs attackDominateCity(long roleId, GamePb8.AttackDominateCityRq req) {
        ...
        if (CheckNull.nonEmpty(heroIdList)) {
            armCount = heroIdList.stream().filter(partnerHero -> !HeroUtil.isEmptyPartner(partnerHero)).
                    mapToInt(partnerHero -> partnerHero.getPrincipalHero().getCount()).sum();
        }
        ...
        army.setHeroMedals(heroIdList.stream()
                .map(partnerHero -> medalDataManager.getHeroMedalByHeroIdAndIndex(player, partnerHero.
                        getPrincipalHero().getHeroId(), MedalConst.HERO_MEDAL_INDEX_0))
    ...
    protected String armyHeroString(Army army) {
        ...
        return army.getHero().stream().filter(partnerHeroIdPb -> Objects.nonNull(partnerHeroIdPb))
                .map(partnerHeroIdPb -> new StringBuilder().append(partnerHeroIdPb).append("_").append(partnerHeroIdPb.getCount()))
     ...
     * @return
     */
    protected CommonPb.Report createFightReport(FightLogic fightLogic, Player attackPlayer, Player defendPlayer, Map<Long, Integer> recoverMap, int nowSec, Map<Long, ChangeInfo> changeMap) {
        // 战斗记录
    ...
    protected void subAndRetreatDeadHeroInArmy(Player player, Fighter fighter, Army army, Map<Long, ChangeInfo> changeMap, Map<Long, Integer> recoverMap, DominateSideCity sideCity, int nowSec) {
        ...
        for (CommonPb.PartnerHeroIdPb partnerHeroIdPb : army.getHero()) {
            ...
            if (hero.getCount() <= 0) {
                ...
                deadHero.add(partnerHeroIdPb);
            } else {
                if (Objects.isNull(survivorHero)) survivorHero = new ArrayList<>();
                survivorHero.add(partnerHeroIdPb);
            }
        }
        if (CheckNull.nonEmpty(survivorHero)) {
            ...
            if (Objects.nonNull(deadHero)) {
                ...
                for (CommonPb.PartnerHeroIdPb partnerHeroIdPb : deadHero) {
                    int recoverHp = recoverHpMap.getOrDefault(partnerHeroIdPb.getPrincipleHeroId(), 0);
                    if (recoverHp > 0) deadArmyRecoverMap.put(partnerHeroIdPb.getPrincipleHeroId(), recoverHp);
    ...
    protected void joinProbing(DominateSideCity sideCity, Player player, Army army, int now) {
        int armyState = getDefenceArmyType(getWorldMapPlay(getWorldMapFunction()));
        army.setState(armyState);
        army.setHeroState(player, armyState);
     ...
     * @param nowSec
     */
    protected abstract void attackFightSuccess(FightLogic fightLogic, DominateSideCity sideCity, CommonPb.Report report, Player attackPlayer, Army army,
     ...
     * @param nowSec
     */
    protected abstract void attackFightFailure(FightLogic fightLogic, DominateSideCity sideCity, CommonPb.Report report, Player attackPlayer, Army army,
                                               Player defendPlayer, int nowSec);
